...
@section('content')
    ...
    <p class="opacity-75 uppercase tracking-wide font-bold text-lg text-center mx-16 my-4"><i class="fa fa-shopping-basket"></i> Editar período</p>

    <div>
        <div class="p-4 rounded-b w-full flex justify-center">
          <div class="w-1/3 bg-gray-100 p-4 shadow-md">
            {!! Form::open(['route' => array('periodos.update', $periodo->id),
                'method' => 'PUT',
                'files' => true]) !!}
                <div>
                    <label class="text-sm text-soft-black" for="nombre">Nombre (ejemplo: AG15-EN16 ó FEB-JUL16)</label>
                    {!! Form::text('nombre', $periodo->nombre, array(
